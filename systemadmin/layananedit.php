...
$mainlib = new mainlib();
$mainlib->opendb();
$listtitle = $mainlib->getpagetitle("Add/Edit Layanan Dan Produk",$mainlib->ocn);
...
if(trim($id)!="")
{
	$ors = $mainlib->dbquery("select * from layanan where layanan_id='".$id."'",$mainlib->ocn);
...
function saveData(){
	//updateRTEs();
	if(validateForm()){
		document.frm.action = 'layananact.php';
		document.frm.submit();
	}
}
...
function DelImage()
{
  $.ajax({
    type: "POST",
    data: 'id=' + <?php echo $id; ?> + '&db=layanan',
    url: 'delete_image.php',
    success: function(msg){
      if(msg != ''){
        $('#'+msg).remove();
        location.href = "layananedit.php?id=<?php echo $id; ?>";
      }
    }
  });
}
...
function DelImages(imageid)
{
  $.ajax({
    type: "POST",
    data: 'id=' + imageid + '&db=imagelayanan',
    url: 'delete_image.php',
    success: function(msg){
      if(msg != ''){
        $('#'+msg).remove();
				...
				<tr>
					<td class="menudottedline" align="right">
						...
						<tr valign="middle" align="center">
							...
							<td>
								<a class="toolbar" href="layananlist.php?pgret=1"><img src="images/cancel_f2.png" alt="Cancel" border="0"><br>Cancel</a>
							</td>
						</tr>
						</table>
					</td>
...
<script type='text/javascript'>Calendar.setup({inputField : 'posting_date',  ifFormat : 'y-mm-dd',button : 'calender_date1',align  : '',singleClick : true});</script>
                ...
                <tr class="row5">
                  ...
                  <td>
                  <?php
                                    $query = $mainlib->dbquery("select * from imagelayanan where layanan_id='".$id."'",$mainlib->ocn);

...

{
	...
	$ors = $mainlib->dbquery("select * from profile where profile_id='".$id."'",$mainlib->ocn);
...
//		if(isBlank(end_date,'Entry required in End Date field!')) return false;
...

    {
      ...
      if(isBlank(flUpload2,'Entry required in Image field!')) return false;

      if(flUpload2.value != ''){

        if(flUpload2.value.toLowerCase().indexOf(".jpg")<=-1 && flUpload2.value.toLowerCase().indexOf(".gif")<=-1 && flUpload2.value.toLowerCase().indexOf(".png")<=-1 && flUpload2.value.toLowerCase().indexOf(".swf")<=-1){
...
function saveData(){
	...
	if(validateForm()){
		...
		document.frm.action = 'profileact.php';

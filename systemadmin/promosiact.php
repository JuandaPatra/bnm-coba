...
$mainlib = new mainlib();
$pageedit = "promosiedit.php";
$mainlib->opendb();

$file_dir = realpath("../promosifile/")."/";
...
switch($act)
{
	case "add":
		...
		if(trim($_FILES["flUpload"]["name"])!="")
		{
		  ...
		  $image_small = "promosifile/".$filename_large;
		...
		$mainlib->dbquery("insert into promosi(posting_date,title_id,title_en,resume_id,resume_en,description_id,description_en,source,image_small) values ('$posting_date','$title_id','$title_en','$resume_id','$resume_en','$description_id','$description_en','$source','$image_small')",$mainlib->ocn);
    
    $id_promosi = mysql_insert_id();
    foreach($temp as $jlh)
      {
        $tes = $tes + 1;
        $im = "image" . $tes;
        if(trim($_FILES[$im]["name"])!="")
        {
          ...
          $images = "promosifile/".$filename_large1;
          ...
          $mainlib->dbquery("insert into imagepromosi(promosi_id,file_image) values('$id_promosi','$images')",$mainlib->ocn);
        }
      }
	...
	case "edit":
		...
		$mainlib->dbquery("update promosi set posting_date = '$posting_date', title_id='$title_id', title_en = '$title_en', resume_id = '$resume_id', resume_en = '$resume_en', description_id = '$description_id', description_en = '$description_en', source = '$source'" . $q_image_small . " where promosi_id='".$id."'",$mainlib->ocn);
		...
		foreach($temp as $jlh)
		  {
			...
			if(trim($_FILES[$im]["name"])!="")
			{
			  ...
			  $mainlib->dbquery("insert into imagepromosi(promosi_id,file_image) values('$id','$images')",$mainlib->ocn);
			}
		  }
		...
		if(isset($_REQUEST["hidIDs"]))
		{
			if(trim($_REQUEST["hidIDs"])!="")
			{
				...
				for($i=0;$i<count($arids);$i++)
				{
					...
					$sql .= "promosi_id='".$arids[$i]."'";
				}
				$sql = "DELETE FROM promosi WHERE (".$sql.")";
...
$mainlib->closedb();
header("Location: promosilist.php?pgret=1");
?>
...
$mainlib = new mainlib();
$mainlib->opendb();
$listtitle = $mainlib->getpagetitle("Add/Edit Hubungan Investor",$mainlib->ocn);
...
if(trim($id)!="")
{
	$ors = $mainlib->dbquery("select * from investor where investor_id='".$id."'",$mainlib->ocn);
...
function saveData(){
	//updateRTEs();
	if(validateForm()){
		document.frm.action = 'investoract.php';
		document.frm.submit();
	}
}
...
function DelImage()
{
  $.ajax({
    type: "POST",
    data: 'id=' + <?php echo $id; ?> + '&db=investor',
    url: 'delete_image.php',
    success: function(msg){
      if(msg != ''){
        $('#'+msg).remove();
        location.href = "investoredit.php?id=<?php echo $id; ?>";
      }
    }
  });
}
...
function DelImages(imageid)
{
  $.ajax({
    type: "POST",
    data: 'id=' + imageid + '&db=imageinvestor',
    url: 'delete_image.php',
    success: function(msg){
      if(msg != ''){
        $('#'+msg).remove();
...
function DelFile()
{
  $.ajax({
    type: "POST",
    data: 'id=' + <?php echo $id; ?> + '&db=investorpdf',
    url: 'delete_image.php',
    success: function(msg){
      if(msg != ''){
        $('#'+msg).remove();
				...
				<tr>
					<td class="menudottedline" align="right">
						...
						<tr valign="middle" align="center">
							...
							<td>
								<a class="toolbar" href="investorlist.php?pgret=1"><img src="images/cancel_f2.png" alt="Cancel" border="0"><br>Cancel</a>
							</td>
						</tr>
						</table>
					</td>
...
<script type='text/javascript'>Calendar.setup({inputField : 'posting_date',  ifFormat : 'y-mm-dd',button : 'calender_date1',align  : '',singleClick : true});</script>
                  ...
                  <td>
                    ...
                    if($file_pdf != "")
                    {
                      ...
                      <div id="<?php echo $id; ?>" style="padding-top:5px">
                                            <?php if($file_pdf != "") { ?>
                                            <?php echo $file_pdf; ?><br />
                                          <a href="javascript:void(0)" onclick="DelFile()">delete this image</a>
                                          <?php } ?>
                      </div>
                      <?php
                    }
                    ...
                    {
                      ?>
                      <input type="file" name="flUploadPDF" />
                                        </div>
                                        <div id="<?php echo $id; ?>" style="padding-top:5px">
                                            <?php if($file_pdf != "") { ?>
                                            <?php echo $file_pdf; ?><br />
                ...
                <tr class="row5">
                  ...
                  <td>
                  <?php
                                    $query = $mainlib->dbquery("select * from imageinvestor where investor_id='".$id."'",$mainlib->ocn);

...
function contenteditable_insertNodeAtSelection(contentWindow, insertNode, insertHTML) {
	...
	if ((container.nodeType == 3) && (insertNode.nodeType == 3)) {
		...
		if (range && range.setEnd && afterNode) {
			if ((container.nodeName == "BODY") && (afterNode.nodeName == "BR") && (pos == 0)) {
...
function contenteditable_formatclass(command,value) {
	...
	for (var i=0; i<contentSelection.rangeCount; i++) {
		...
		if ((range.startContainer == range.endContainer) && (range.startOffset != range.endOffset) && (range.startContainer.nodeType == 3)) { // TEXT NODE
		...
		} else if (range.startContainer.nodeName == "BODY") {
			var node;
			if ((range.startContainer.firstChild != range.startContainer.lastChild) || (range.startContainer.firstChild.nodeType != 3) || (range.endContainer.firstChild.nodeType != 3)) {
			...
			for (var j=range.startOffset; j<range.endOffset; j++) {
				var childnode = range.startContainer.childNodes[j];

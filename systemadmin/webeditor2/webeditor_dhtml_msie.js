...
function HardCoreWebEditor(rootpath, language, name, value, value_htmlencoded, stylesheet, showhtml, manager, onEnter, onShiftEnter, onCtrlEnter, onAltEnter, toolbar, width, height, format, encoding, direction) {
	...
	if (! WebEditor.instance) {
		WebEditor.instance = new Array();
		this.instance = 1;
	} else {
		this.instance = WebEditor.instance.length;
	}
	WebEditor.instance[this.instance] = this;
...
function WebEditorGetContent(id) {
	...
	if (id) {
		var editor;
		for (var i=1; i<=WebEditor.instance.length; i++) {
			if (editor = WebEditor.instance[i]) {
				if (editor.editor == id) {
					if (editor.HTML) {
						editor.onButtonHTML();
					}
		...
		if (editor = WebEditor.instance[contenteditable_focused]) {
			if (editor.HTML) {
				editor.onButtonHTML();
			}
			content = editor.DHTMLSafe.DOM.body.innerHTML;
...
function onLoad() {
	...
	var instance;
	for (instance in WebEditor.instance) if (typeof(WebEditor.instance[instance]) != "function") {
		if (! WebEditor.instance[instance].initialised) {
			WebEditor.instance[instance].initialised = true;
		
			if (select = eval('document.all.'+WebEditor.instance[instance].editor+'_fontstyle'+WebEditor.instance[instance].instance)) { //Populate QuickFormat Box
				var obj = new ActiveXObject("DEGetBlockFmtNamesParam.DEGetBlockFmtNamesParam");
				WebEditor.instance[instance].DHTMLSafe.ExecCommand(DECMD_GETBLOCKFMTNAMES,OLECMDEXECOPT_DODEFAULT,obj);
			...
			if (WebEditor.instance[instance].ContentItem.value.length) {
				WebEditor.instance[instance].DHTMLSafe.DOM.body.innerHTML = WebEditor.instance[instance].ContentItem.value;
			} else {
				WebEditor.instance[instance].DHTMLSafe.DOM.body.innerHTML = " ";
			}

			WebEditor.instance[instance].refreshToolbar();
		}
	}
}
...
function onSubmit() {
	if ((typeof(WebEditor) != "undefined") && (typeof(WebEditor.instance) != "undefined")) {
		var editor;
		for (var i=1; i<=WebEditor.instance.length; i++) {
			if (editor = WebEditor.instance[i]) {
				if (editor.HTML) {
...
function contextMenuAction(action) {
	if (this.ContextMenu[action].action == "about") {
		alert("Asbru Web Content Editor"+"\n"+"(C) 2002-2008 - Asbru Ltd."+"\n"+"www.asbrusoft.com");
	} else {
...
//	document.writeln('<div align="left"><font size="-1">Asbru Web Content Editor - (C) 2002-2008 - Asbru Ltd. - <a href="http://www.asbrusoft.com" target="_blank">www.asbrusoft.com</a></font></div>');

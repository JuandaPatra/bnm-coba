...
for (var i=0; i<scripts.length; i++) {
	if (scripts[i].src && scripts[i].src.match(/\/webeditor2\.js(\?.*)?$/)) {
		if (scripts[i].src.match(/^(http:|https:|\/|\.\.)/)) {
			webeditor.rootpath = scripts[i].src.replace(/webeditor2\.js(\?.*)?$/,'').replace(/^(http|https):\/\/[^\/]+/, "");
		} else {
			webeditor.rootpath += scripts[i].src.replace(/webeditor2\.js(\?.*)?$/,'').replace(/^(http|https):\/\/[^\/]+/, "");
		}
	}
...
//	webeditor.type = "textarea";
} else if (browserIs("Safari") && browserIs("Win") && (browserIs("Safari/5"))) {
	webbrowser = "Safari";
	webeditor.minorVersion = parseFloat(versionIs());
	webeditor.majorVersion = parseInt(webeditor.minorVersion);
	webeditor.type = "safari";

		...
		try {
			if (iframe && iframe.contentWindow && iframe.contentWindow.document && iframe.contentWindow.document && iframe.contentWindow.document.body) {
				if (((iframe.className == 'webeditor_contenteditable') || (iframe.className == 'webeditor_contenteditable_disabled'))) {
					try {
						iframe.contentWindow.detachEvent("onfocus", contenteditable_onfocus[i]);
					}  catch (e) {
					}
					try {
						iframe.contentWindow.detachEvent("onblur", contenteditable_onblur[i]);
					}  catch (e) {
					}
					try {
						iframe.contentWindow.document.body.detachEvent("onfocus", contenteditable_onfocus[i]);
					}  catch (e) {
					}
					try {
						iframe.contentWindow.document.body.detachEvent("onblur", contenteditable_onblur[i]);
					}  catch (e) {
					}
				}
			}
...
//						if (! contenteditable_onblur[i]) contenteditable_onblur[i] = new Function('event', 'webeditor_onblur();webeditor_refreshToolbar(false, 1);');
						...
						if (webeditor.contextmenus) iframe.contentWindow.document.attachEvent("oncontextmenu", contenteditable_contextmenu);
...
function contenteditable_selection_range_parentNode(contentRange) {
	...
	if (contentRange && contentRange.parentElement) {
		try {
			...
			if ((parentnode.innerHTML.indexOf(contentRange.htmlText) < 0) && (parentnode.nodeName != "BODY")) {
	...
	if ((! insertNode.childNodes) || (! insertNode.childNodes.length)) {
		...
		try {
			...
			if (expandednode) {
				...
				while ((parentnode.nodeName != "SPAN") && (parentnode.nodeName != "BODY") && (parentnode.nodeName != "HTML")) {
					parentnode = parentnode.parentNode;
				}
...
function contenteditable_setAttribute(node, attribute, value) {
	...
	if ((node.nodeName == "INPUT") && (attribute == "type") && (contenteditable_getAttribute(node, attribute) != value)) {
		...
		if (node.outerHTML.match(/^(<INPUT[^>]+type=)("[a-z]*")([^>]*>)$/gi)) {
			...
			node.outerHTML = node.outerHTML.replace(/^(<INPUT[^>]+)([^>]*>)$/gi, "$1 type="+value.replace(/\$/g,"$$$$")+" $3");
		}
	}
...
//	return node.getAttribute(attribute,2).replace(/&amp;/gi, "&");
// MSIE element.getAttribute(attribute) may not work properly - changes href/src from relative to absolute + returns unspecified default values
	var value = "";
	if (node) {
		if ((node.outerHTML) || (typeof(node) == "string")) {
			var node_outerHTML;
			if (node.outerHTML) {
...
// MSIE may change table cell align=center to align=middle
	if ((node.nodeName == "TD") && (attribute == "align") && (value == "middle")) {
		value = "center";
	}
// MSIE may change table cell valign=middle to valign=center
	if ((node.nodeName == "TD") && (attribute == "vAlign") && (value == "center")) {
		value = "middle";
	}
	return value;
}
...
function contenteditable_webeditor_clipboard_paste() {
	if (webeditor.clipboard && webeditor.clipboardHTML && webeditor.clipboardText && (webeditor.clipboardText == window.clipboardData.getData("Text"))) {
		return true;
	} else {
		return false;
	}

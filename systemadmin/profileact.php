...
$pageedit = "profileedit.php";

$mainlib->opendb();



$file_dir = realpath("../profilefile/")."/";
...

{
	...
	case "add":
...

		{
			...
			$file_image = "profilefile/".$filename;
...

		{
			...
			$file_image2 = "profilefile/".$filename2;
		...
		$mainlib->dbquery("insert into profile(start_date,end_date,title,file_image_id,file_image_en,link,caption,sort) values ('$start_date','$end_date','$title','$file_image','$file_image2','$link','$caption','$sort')",$mainlib->ocn);
    ...
    $file_image2 ='';
		...
		if($file_image == ''){
 			...
 			$mainlib->dbquery("update profile set title = '$title', start_date='$start_date', end_date = '$end_date', link = '$link', caption = '$caption',sort = '$sort' where slider_id='".$id."'",$mainlib->ocn);

		}else{
			

			$mainlib->dbquery("update profile set title = '$title', start_date='$start_date', end_date = '$end_date', link = '$link', caption = '$caption',sort = '$sort', file_image_id='".$file_image."' where slider_id='".$id."'",$mainlib->ocn);

		}


		if($file_image2 !=''){
			$mainlib->dbquery("update profile set  file_image_en='".$file_image2."' where slider_id='".$id."'",$mainlib->ocn);			
		}
...

		{
...

			{
...

				{
					...
					$sql .= "profile_id='".$arids[$i]."'";

				}

				$sql = "DELETE FROM profile WHERE (".$sql.")";
...
header("Location: profilelist.php?pgret=1");

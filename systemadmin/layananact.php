...
$mainlib = new mainlib();
$pageedit = "layananedit.php";
$mainlib->opendb();

$file_dir = realpath("../layananfile/")."/";
...
switch($act)
{
	case "add":
		...
		if(trim($_FILES["flUpload"]["name"])!="")
		{
		  ...
		  $image_small = "layananfile/".$filename_large;
		...
		$mainlib->dbquery("insert into layanan(posting_date,title_id,title_en,resume_id,resume_en,description_id,description_en,source,image_small) values ('$posting_date','$title_id','$title_en','$resume_id','$resume_en','$description_id','$description_en','$source','$image_small')",$mainlib->ocn);
    
    $id_layanan = mysql_insert_id();
    foreach($temp as $jlh)
      {
        $tes = $tes + 1;
        $im = "image" . $tes;
        if(trim($_FILES[$im]["name"])!="")
        {
          ...
          $images = "layananfile/".$filename_large1;
          ...
          $mainlib->dbquery("insert into imagelayanan(layanan_id,file_image) values('$id_layanan','$images')",$mainlib->ocn);
        }
      }
	...
	case "edit":
		...
		$mainlib->dbquery("update layanan set posting_date = '$posting_date', title_id='$title_id', title_en = '$title_en', resume_id = '$resume_id', resume_en = '$resume_en', description_id = '$description_id', description_en = '$description_en', source = '$source'" . $q_image_small . " where layanan_id='".$id."'",$mainlib->ocn);
		...
		foreach($temp as $jlh)
		  {
			...
			if(trim($_FILES[$im]["name"])!="")
			{
			  ...
			  $mainlib->dbquery("insert into imagelayanan(layanan_id,file_image) values('$id','$images')",$mainlib->ocn);
			}
		  }
		...
		if(isset($_REQUEST["hidIDs"]))
		{
			if(trim($_REQUEST["hidIDs"])!="")
			{
				...
				for($i=0;$i<count($arids);$i++)
				{
					...
					$sql .= "layanan_id='".$arids[$i]."'";
				}
				$sql = "DELETE FROM layanan WHERE (".$sql.")";
...
$mainlib->closedb();
header("Location: layananlist.php?pgret=1");
?>